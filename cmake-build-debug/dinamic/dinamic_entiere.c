...
// Maximum length for each number word
#define MAX_WORD_LENGTH 100

// Array to store numbers read from file
...
int loadFrenchNumbers(const char *filename) {
    ...
    // Allocate initial memory for array of strings
    ...
    while (fgets(line, sizeof(line), file) && index < 1000) {
        ...
        // Allocate memory for this number and copy it
...
#define CHIFFRES_MAX  (NOMBRE_UNITES*3) // Le nombre maximal de chiffres dans le nombre que l'utilisateur peut entrer correspond
// au nombre d'unités.
...
int main(void) {
    ...
    if (erreur == AUCUNE_ERREUR) { // Vérifier s'il n'y a pas d'erreurs.
        convertir_nombre_en_lettres(entree, resultat);
        printf("Le nombre %s en lettres est : %s\n", entree, resultat);
    } else { // Si une erreur existe, indiquer quelle erreur a été rencontrée pour informer l'utilisateur.
        switch (erreur) {
...
void convertir_nombre_entiere(long long nombre, char *resultat) {
    if (nombre < 1000) { // Si le nombre est inférieur à 1000,
        strcat(resultat, frenchNumbers[nombre]);//utiliser directement la valeur correspondante dans le tableau frenchNumbers.
        return;
    }

    int premiere_partie = 1; // Pour indiquer si l'on est dans la première partie du nombre (les 3 premiers chiffres).

    for (int i = 0; i < NOMBRE_UNITES; i++) { // Commence par la plus grande unité que l'on ait (par exemple, milliard, million, etc.).

        long long valeur_courante = nombre / diviseurs_unites[i]; // Déterminer combien de fois une unité est contenue dans le nombre.

        if (valeur_courante > 0) {
            if (!premiere_partie) {
                strcat(resultat, " "); // Si ce n'est pas la première partie, ajouter un espace.
            }
            premiere_partie = 0; // Après la première unité, on est sûr qu'on n'est plus dans la première partie du nombre.

            if (valeur_courante < 1000) {
                strcat(resultat, frenchNumbers[valeur_courante]); // Ajouter la valeur correspondante en lettres pour les nombres inférieurs à 1000.
            }
            ...
            if (noms_unites[i][0] != '\0') {
                ...
                if (valeur_courante > 1 && strcmp(noms_unites[i], "mille") != 0) { // Gérer le cas du pluriel, sauf pour "mille"
                    strcat(resultat, "s"); // Ajouter "s" pour les unités au pluriel (par exemple, milliards).
                }
            }
        }
        nombre %= diviseurs_unites[i]; // Réduire le nombre pour traiter la partie suivante (les unités restantes après division).
    }
}
