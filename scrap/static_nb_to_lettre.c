...
// Simplified function to handle large numbers (>999)
void convertir_grand_nombre(long long nombre, char *resultat) {
    if (nombre < 1000) {
    ...
    for (int i = 0; i < NOMBRE_UNITES; i++) {
        ...
        if (valeur_courante > 0) {
            ...
            // Handle numbers less than 1000 using array
            strcat(resultat, frenchNumbers[valeur_courante]);

            // Add unit name (billion, million, thousand)
...
void convertir_nombre_en_lettres(const char *entree, char *resultat) {
    ...
    if (nombre == 0) {
        ...
        convertir_grand_nombre(nombre, resultat);
    }
...
void convertir_decimales(const char *partie_decimale, char *resultat) {
    ...
    for (int i = 0; partie_decimale[i] != '\0'; i++) {
        strcat(resultat, " ");
        int digit = partie_decimale[i] - '0';
        strcat(resultat, frenchNumbers[digit]);
    }
}
...
void extraire_parties_nombre(const char *entree, char *partie_entiere, char *partie_decimale) {
    ...
    // Skip leading zeros while keeping at least one zero before decimal point
...
int valider_entree(const char *entree) {
    ...
    // Don't count leading zeros in the length check
